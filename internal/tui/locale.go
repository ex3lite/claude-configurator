...
var permissionOptionKeys = map[string]string{
	...
	"Bash(git reset *)":           "git-reset",
	"Bash(npm publish *)":         "npm-publish",
	"Bash(cargo publish *)":       "cargo-publish",
	"Edit(/.github/workflows/**)": "workflows",
	"Read(./.env)":                "env",
	"Read(./.env.*)":              "env-variants",
...
func (m *Model) optionLabel(specID, option string) string {
	...
	if option == customChoice {
		if text := translations[m.language]["option."+specID+".custom"]; text != "" {
			return text
		}
		if strings.Contains(specID, "model") {
			return m.tr("option.custom_model")
		}
	...
	if specID == "statusline-theme" && strings.HasPrefix(option, "claude-config statusline --theme ") {
		return m.tr("option.statusline-theme." + strings.TrimPrefix(option, "claude-config statusline --theme "))
	}
	if key := permissionOptionKeys[option]; key != "" {
		if text := translations[m.language]["option."+specID+"."+key]; text != "" {
			return text
		}
	}
...
var translations = map[uiLanguage]map[string]string{
	languageEN: {
		...
		"category.Claude CLI.description":  "Tune agent limits, hidden CLI flags, and the Claude-style status bar.",
		...
		"option.statusline-theme.auto":             "Auto · best terminal colors",
		"option.statusline-theme.nerd":             "Claude Icons · Nerd Font v3",
		"option.statusline-theme.claude":           "Claude clay · true color",
		...
		"option.permission-allow.cargo-test":       "Run Cargo tests",
		"option.permission-allow.pytest":           "Run pytest",
		"option.permission-allow.github":           "Fetch github.com",
		"option.permission-ask.git-push":           "Confirm Git pushes",
		"option.permission-ask.git-reset":          "Confirm Git resets",
		"option.permission-ask.npm-publish":        "Confirm npm publishing",
		"option.permission-ask.cargo-publish":      "Confirm Cargo publishing",
		"option.permission-ask.workflows":          "Confirm workflow edits",
		"option.permission-deny.env":               "Block the .env file",
		"option.permission-deny.env-variants":      "Block .env variants",
		"option.permission-deny.secrets":           "Block the secrets directory",
		"option.permission-deny.ssh":               "Block SSH files",
		"option.permission-deny.force-push":        "Block force pushes",
		...
		"list.empty.title":                         "Nothing configured here yet",
		"list.empty.unset":                         "The “%s” scope does not store this key.",
		"list.empty.stored":                        "The “%s” scope stores an empty list. Press U to remove the key and restore inheritance.",
		"list.effective":                           "EFFECTIVE VALUES · %s",
		"list.default":                             "There is no inherited list. Claude Code keeps its built-in behavior.",
		"list.add_options":                         "Choose a ready option",
		"list.add_custom":                          "Add a custom value",
		"list.help":                                "a add · Enter/e edit · d delete · u reset to inherit · Esc done",
		...
		"update.confirm.title":                     "Claude Configurator update",
		"update.confirm.text":                      "Version %s is available. Download it, verify its SHA-256 checksum, replace this binary, and restart now?\n\nYour Claude Code settings are never uploaded.",
		"update.confirm.help":                      "Enter/y update now · Esc/n later",
		"update.installing.title":                  "Installing update",
		"update.installing.text":                   "Downloading version %s and verifying its SHA-256 checksum…",
		"update.installing.help":                   "Keep this terminal open. Claude Configurator will restart automatically.",
		"update.failed":                            "Update failed; the current version was not changed: %s",
		"update.later":                             "Update postponed",
		...
		"status.integer_min":                       "Enter a whole number greater than or equal to %d",
		"status.integer_range":                     "Enter a whole number from %d to %d",
		...
		"status.updated":                           "Claude Configurator updated to %s",
	},
	languageRU: {
		...
		"category.Claude CLI":              "Claude CLI",
		...
		"category.Claude CLI.description":  "Лимиты агентов, скрытые флаги и статус-бар в стиле Claude.",
		...
		"option.custom_value":                      "Другое значение…",
		...
		"option.ui-language.ru":                    "Русский",
		"option.ui-language.zh-CN":                 "简体中文",
		"option.statusline-theme.auto":             "Авто · лучшие цвета терминала",
		"option.statusline-theme.nerd":             "Claude с иконками · Nerd Font v3",
		"option.statusline-theme.claude":           "Claude clay · true color",
		...
		"option.permission-allow.cargo-test":       "Запускать Cargo-тесты",
		"option.permission-allow.pytest":           "Запускать pytest",
		"option.permission-allow.github":           "Открывать github.com",
		"option.permission-ask.git-push":           "Подтверждать Git push",
		"option.permission-ask.git-reset":          "Подтверждать Git reset",
		"option.permission-ask.npm-publish":        "Подтверждать npm publish",
		"option.permission-ask.cargo-publish":      "Подтверждать Cargo publish",
		"option.permission-ask.workflows":          "Подтверждать правки workflow",
		"option.permission-deny.env":               "Блокировать файл .env",
		"option.permission-deny.env-variants":      "Блокировать варианты .env",
		"option.permission-deny.secrets":           "Блокировать каталог secrets",
		"option.permission-deny.ssh":               "Блокировать SSH-файлы",
		"option.permission-deny.force-push":        "Блокировать force push",
		...
		"editor.custom_value":                      "Своё значение: %s",
		...
		"list.empty.title":                         "Здесь пока ничего не настроено",
		"list.empty.unset":                         "На уровне «%s» этот ключ не записан.",
		"list.empty.stored":                        "На уровне «%s» сохранён пустой список. Нажмите U, чтобы удалить ключ и вернуть наследование.",
		"list.effective":                           "ИТОГОВЫЕ ЗНАЧЕНИЯ · %s",
		"list.default":                             "Наследуемого списка нет — Claude Code использует встроенное поведение.",
		"list.add_options":                         "Выбрать готовый вариант",
		"list.add_custom":                          "Добавить своё значение",
		"list.help":                                "a добавить · Enter/e изменить · d удалить · u наследовать · Esc готово",
		...
		"update.confirm.title":                     "Обновление Claude Configurator",
		"update.confirm.text":                      "Доступна версия %s. Скачать её, проверить SHA-256, заменить текущий бинарник и перезапустить приложение?\n\nНастройки Claude Code никуда не отправляются.",
		"update.confirm.help":                      "Enter/y обновить сейчас · Esc/n позже",
		"update.installing.title":                  "Установка обновления",
		"update.installing.text":                   "Скачиваем версию %s и проверяем её SHA-256…",
		"update.installing.help":                   "Не закрывайте терминал. Claude Configurator перезапустится автоматически.",
		"update.failed":                            "Обновление не установлено, текущая версия не изменена: %s",
		"update.later":                             "Обновление отложено",
		...
		"status.integer_min":                       "Введите целое число не меньше %d",
		"status.integer_range":                     "Введите целое число от %d до %d",
		...
		"status.updated":                           "Claude Configurator обновлён до версии %s",
		...
		"spec.subagent-depth.label":      "Глубина сабагентов",
		"spec.subagent-total.label":      "Сабагентов за сессию",
		"spec.subagent-concurrent.label": "Одновременные сабагенты",
		"spec.tool-concurrency.label":    "Параллельность tools",
		"spec.new-init.label":            "Интерактивный /init",
		"spec.task-list-id.label":        "Общий task list ID",
		"spec.statusline-theme.label":    "Тема статус-бара",
		"spec.statusline-refresh.label":  "Обновление статус-бара",
		"spec.statusline-padding.label":  "Отступ статус-бара",
		"spec.statusline-vim.label":      "Скрыть Vim-индикатор",
		"spec.statusline-type.label":     "Тип команды status line",
		...
		"spec.subagent-depth.description":      "Сколько уровней сабагентов разрешено ниже основного чата. По умолчанию 1; нужен Claude Code 2.1.217+.",
		"spec.subagent-total.description":      "Общий лимит созданных сабагентов за сессию. По умолчанию 200; нужен Claude Code 2.1.212+.",
		"spec.subagent-concurrent.description": "Сколько сабагентов может работать одновременно. По умолчанию 20; нужен Claude Code 2.1.217+.",
		"spec.tool-concurrency.description":    "Параллельность read-only tools и сабагентов. По умолчанию 10.",
		"spec.new-init.description":            "Превращает /init в мастер настройки CLAUDE.md, skills и hooks.",
		"spec.task-list-id.description":        "Один ID позволяет нескольким сессиям использовать общий список задач.",
		"spec.statusline-theme.description":    "Панель над footer: отдельно основная модель, сабагенты и advisor, затем проект, Git, контекст и лимиты 5h/7d.",
		"spec.statusline-refresh.description":  "Интервал обновления status line в секундах. Для countdown достаточно 60.",
		"spec.statusline-padding.description":  "Дополнительный горизонтальный отступ status line.",
		"spec.statusline-vim.description":      "Скрывает встроенный индикатор режима Vim под prompt.",
		"spec.statusline-type.description":     "Внутренний тип команды status line Claude Code.",
		...
		"spec.subagent-depth.purpose":      "Значение 2+ позволяет сабагентам делить собственную работу, но каждый новый уровень может умножить число агентов и расход ресурсов.",
		"spec.subagent-total.purpose":      "Уменьшайте лимит для контроля длинных автономных запусков; повышайте только для сценариев, которым действительно мало 200 задач.",
		"spec.subagent-concurrent.purpose": "Ограничивает пиковую нагрузку процессов и API отдельно от общего лимита сабагентов.",
		"spec.tool-concurrency.purpose":    "Повышение ускоряет независимое чтение на мощной машине; снижение уменьшает нагрузку и всплески rate limit.",
		"spec.new-init.purpose":            "Полезно при запуске нового проекта, когда вы хотите выбрать создаваемые файлы, а не принять автоматический CLAUDE.md.",
		"spec.task-list-id.purpose":        "Назначьте одинаковый ID сотрудничающим терминальным сессиям, чтобы они координировались через один task list.",
		"spec.statusline-theme.purpose":    "Сразу видно, какая модель отвечает за каждую роль и когда сбросятся реальные лимиты. Тема с иконками появляется автоматически при наличии Nerd Font.",
		"spec.statusline-refresh.purpose":  "Поддерживает часы и время сброса актуальными, даже когда основной чат ждёт фоновых агентов.",
		"spec.statusline-padding.purpose":  "Выравнивает панель с prompt и раскладкой терминала, не меняя содержимое.",
		"spec.statusline-vim.purpose":      "Убирает дублирование, если статус-бар уже показывает состояние Vim.",
		"spec.statusline-type.purpose":     "Записывается автоматически при выборе встроенного статус-бара.",
	...
	languageZH: {
		...
		"category.Claude CLI.description":  "设置代理限制、隐藏标志和 Claude 风格状态栏。",
		...
		"option.ui-language.ru":                    "Русский",
		"option.ui-language.zh-CN":                 "简体中文",
		"option.statusline-theme.auto":             "自动 · 最佳终端颜色",
		"option.statusline-theme.nerd":             "Claude 图标 · Nerd Font v3",
		"option.statusline-theme.claude":           "Claude clay · 真彩色",
		...
		"option.permission-allow.cargo-test":       "运行 Cargo 测试",
		"option.permission-allow.pytest":           "运行 pytest",
		"option.permission-allow.github":           "访问 github.com",
		"option.permission-ask.git-push":           "确认 Git push",
		"option.permission-ask.git-reset":          "确认 Git reset",
		"option.permission-ask.npm-publish":        "确认 npm publish",
		"option.permission-ask.cargo-publish":      "确认 Cargo publish",
		"option.permission-ask.workflows":          "确认工作流编辑",
		"option.permission-deny.env":               "阻止读取 .env",
		"option.permission-deny.env-variants":      "阻止读取 .env 变体",
		"option.permission-deny.secrets":           "阻止读取 secrets 目录",
		"option.permission-deny.ssh":               "阻止读取 SSH 文件",
		"option.permission-deny.force-push":        "阻止强制推送",
		"app.source":                               "应用",
		...
		"editor.custom_value":                      "自定义%s",
		"editor.edit":                              "编辑 %s",
		"editor.apply":                             "Enter 应用 · Esc 取消 · ←→ 移动",
		"choice.help":                              "↑↓ 选择 · Enter 应用 · Esc 取消",
		"choice.inherit_help":                      "“默认 / 继承”会删除“%s”层级中的键，而不是写入一个伪默认值。",
		"list.empty.title":                         "这里尚未配置任何内容",
		"list.empty.unset":                         "“%s”层级没有保存此键。",
		"list.empty.stored":                        "“%s”层级保存了空列表。按 U 删除该键并恢复继承。",
		"list.effective":                           "生效值 · %s",
		"list.default":                             "没有可继承的列表，Claude Code 将使用内置行为。",
		"list.add_options":                         "选择预设选项",
		"list.add_custom":                          "添加自定义值",
		"list.help":                                "a 添加 · Enter/e 编辑 · d 删除 · u 恢复继承 · Esc 完成",
		"list.maximum":                             "最多：%d 项",
		"diff.more":                                "… 还有 %d 项更改",
		...
		"update.confirm.title":                     "Claude Configurator 更新",
		"update.confirm.text":                      "发现新版本 %s。是否立即下载、验证 SHA-256、替换当前程序并自动重启？\n\nClaude Code 设置不会被上传。",
		"update.confirm.help":                      "Enter/y 立即更新 · Esc/n 稍后",
		"update.installing.title":                  "正在安装更新",
		"update.installing.text":                   "正在下载版本 %s 并验证 SHA-256…",
		"update.installing.help":                   "请保持终端打开，Claude Configurator 将自动重启。",
		"update.failed":                            "更新失败，当前版本未被修改：%s",
		"update.later":                             "已推迟更新",
		...
		"status.integer_min":                       "请输入不小于 %d 的整数",
		"status.integer_range":                     "请输入 %d 到 %d 之间的整数",
		...
		"status.updated":                           "Claude Configurator 已更新到 %s",
		...
		"spec.subagent-depth.label":      "子代理生成深度",
		"spec.subagent-total.label":      "每会话子代理数",
		"spec.subagent-concurrent.label": "并发子代理",
		"spec.tool-concurrency.label":    "工具并发数",
		"spec.new-init.label":            "交互式 /init",
		"spec.task-list-id.label":        "共享任务列表 ID",
		"spec.statusline-theme.label":    "状态栏主题",
		"spec.statusline-refresh.label":  "状态栏刷新",
		"spec.statusline-padding.label":  "状态栏缩进",
		"spec.statusline-vim.label":      "隐藏 Vim 指示器",
		"spec.statusline-type.label":     "状态栏命令类型",
		...
		"spec.subagent-depth.description":      "主对话下允许的子代理层数。默认 1，需要 Claude Code 2.1.217+。",
		"spec.subagent-total.description":      "每个会话可创建的子代理总数。默认 200，需要 Claude Code 2.1.212+。",
		"spec.subagent-concurrent.description": "可同时运行的子代理数量。默认 20，需要 Claude Code 2.1.217+。",
		"spec.tool-concurrency.description":    "只读工具和子代理的并行数量。默认 10。",
		"spec.new-init.description":            "让 /init 以向导方式配置 CLAUDE.md、skills 和 hooks。",
		"spec.task-list-id.description":        "相同 ID 让多个会话共享同一个任务列表。",
		"spec.statusline-theme.description":    "输入区上方分别显示主模型、子代理模型和 advisor，再显示项目、Git、上下文及 5h/7d 限制。",
		"spec.statusline-refresh.description":  "状态栏刷新间隔（秒）；倒计时使用 60 秒即可。",
		"spec.statusline-padding.description":  "Claude Code 在状态栏前添加的水平缩进。",
		"spec.statusline-vim.description":      "隐藏提示符下方内置的 Vim 模式指示器。",
		"spec.statusline-type.description":     "Claude Code 内部的状态栏命令类型。",
		...
		"spec.subagent-depth.purpose":      "设置为 2 或更高可让子代理继续拆分工作，但每增加一层都可能成倍增加代理和资源消耗。",
		"spec.subagent-total.purpose":      "降低该值可限制长时间自治运行；只有确实需要超过 200 个委派任务时才提高。",
		"spec.subagent-concurrent.purpose": "独立限制进程和 API 的峰值压力，不改变整场会话的子代理总上限。",
		"spec.tool-concurrency.purpose":    "强大机器可提高以加速独立读取；降低可减少负载和速率峰值。",
		"spec.new-init.purpose":            "初始化新项目时可选择生成哪些文件，而不是直接接受自动 CLAUDE.md。",
		"spec.task-list-id.purpose":        "为协作终端会话设置同一 ID，使它们通过一个任务列表协调进度。",
		"spec.statusline-theme.purpose":    "无需解析终端即可看清每个角色使用的模型和真实限制重置时间；安装 Nerd Font 后自动解锁图标主题。",
		"spec.statusline-refresh.purpose":  "即使主会话等待后台代理，也能保持时钟和重置时间更新。",
		"spec.statusline-padding.purpose":  "让状态栏与提示符和终端布局对齐，不修改其内容。",
		"spec.statusline-vim.purpose":      "自定义状态栏已显示 Vim 状态时避免重复。",
		"spec.statusline-type.purpose":     "选择内置状态栏时自动写入。",
