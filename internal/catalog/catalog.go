...
var Specs = []Spec{
	...
	{ID: "fallback-models", Path: "fallbackModel", Label: "Fallback models", Category: "Models", Kind: List, Options: []string{"default", "fable", "sonnet", "opus", "haiku", "sonnet[1m]", "opus[1m]", "claude-fable-5[1m]", "claude-sonnet-5"}, AllowCustom: true, MaxItems: 3, Description: "Ordered fallback chain of up to three unique aliases or model IDs.", Purpose: "Keep work moving when the preferred model cannot serve a request and Claude Code needs another allowed model."},
	...
	{ID: "statusline-theme", Path: "statusLine.command", Label: "Status bar theme", Category: "Claude CLI", Kind: Enum, Options: []string{"claude-config statusline --theme auto", "claude-config statusline --theme nerd", "claude-config statusline --theme claude", "claude-config statusline --theme ansi", "claude-config statusline --theme mono"}, AllowCustom: true, Description: "Claude-style bar with separate primary, subagent, and advisor model roles, project context, and real 5-hour and 7-day usage windows.", Purpose: "See which model handles each role and when limits reset without parsing terminal output. The icon theme unlocks automatically when a Nerd Font is installed."},
	...
	{ID: "permission-allow", Path: "permissions.allow", Label: "Allow rules", Category: "Permissions", Kind: List, Options: []string{"Bash(npm test)", "Bash(go test ./...)", "Bash(cargo test *)", "Bash(pytest *)", "WebFetch(domain:github.com)"}, AllowCustom: true, Description: "Tool rules that run without a confirmation prompt. Lists merge across scopes.", Purpose: "Reduce prompt fatigue for commands and paths you already trust."},
	{ID: "permission-ask", Path: "permissions.ask", Label: "Ask rules", Category: "Permissions", Kind: List, Options: []string{"Bash(git push *)", "Bash(git reset *)", "Bash(npm publish *)", "Bash(cargo publish *)", "Edit(/.github/workflows/**)"}, AllowCustom: true, Description: "Tool rules that always require your confirmation. Lists merge across scopes.", Purpose: "Keep sensitive actions visible even when a broader rule would otherwise allow them."},
	{ID: "permission-deny", Path: "permissions.deny", Label: "Deny rules", Category: "Permissions", Kind: List, Options: []string{"Read(./.env)", "Read(./.env.*)", "Read(./secrets/**)", "Read(~/.ssh/**)", "Bash(git push --force *)"}, AllowCustom: true, Description: "Tool rules Claude Code must always reject. Lists merge across scopes.", Purpose: "Block commands, tools, or paths that Claude must never access."},
