...
- 使用类型化控件设置嵌套代理深度、子代理总数/并发数、工具并发、
  交互式 `/init` 和共享任务列表。
- Claude 风格状态栏可显示剩余上下文、真实 5 小时/7 天限额、系统时区内的
  精确重置时间和倒计时。
...
系统时区内的精确重置日期和时间，以及易读倒计时，例如：
`今天 17:00 (UTC+8) · 还有 3小时23分钟`。宽终端会并排显示两个窗口，
窄终端则分行显示。首行还包含模型、项目、Git 分支、剩余上下文和本地时间；
有数据时继续显示 session、agent、effort、thinking、fast、Vim 和输出风格。
